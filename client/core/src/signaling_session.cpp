...
std::optional<Envelope> SignalingSession::relay(std::string type,std::string payload){if(!joined_||(type!="session-description"&&type!="ice-candidate"&&type!="restart-ice"))return std::nullopt;return command(std::move(type),room_id_,std::move(payload));}
